 ...
 */
public class ControladorAsociarLibrosArriendo implements ActionListener{
    private Libro modelLibro;
    private Arriendos modelArriendo;
    private AsociarArriendoLibroSql sql;
    private GestionarAsocLibVentas vista;

    private Libro libTemp = null;
    public ControladorAsociarLibrosArriendo(Libro modelLibro, Arriendos modelArriendo, AsociarArriendoLibroSql sql, GestionarAsocLibVentas vista) {
        ...
        System.out.println("[MODELO DE VENTAS -> "+this.modelArriendo.toString()+"]");
    ...
    public void muestrate() {
        ...
        this.vista.setTitle("Biblioteca Fastdevelopment | Gestión añadir libros a arriendo");
        this.vista.txt_tituloTabla.setText("Añadir libros a arriendo: " + this.modelArriendo.getCodigo());
        ...
        this.vista.campo_numSerie.setText(String.valueOf(modelArriendo.getCodigo()));
    }
    ...
    private void asociar() {
        try {
            if (!libTemp.equals(null)) {
                if (sql.insert(modelArriendo.getId(), libTemp.getId())) {
    ...
    private void quitar() {
        try {
            if (!libTemp.equals(null)) {
                if (sql.delete(modelArriendo.getId(), libTemp.getId())) {
