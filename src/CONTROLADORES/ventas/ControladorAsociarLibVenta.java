 ...
 */
public class ControladorAsociarLibVenta implements ActionListener{
    private Libro modelLibro;
    private Ventas modelVenta;
    private AsociarLibrosVentasSql sql;
    private GestionarAsocLibVentas vista;

    private Libro libTemp = null;
    private DefaultTableModel modeloTablaRescatado = null;
    
    public ControladorAsociarLibVenta(Libro modelLibro, Ventas modelVenta, AsociarLibrosVentasSql sql, GestionarAsocLibVentas vista) {
        ...
        System.out.println("[MODELO DE VENTAS -> "+this.modelVenta.toString()+"]");
    ...
    public ControladorAsociarLibVenta() {
    }

    public void muestrate() {
        this.vista.setVisible(true);
        this.vista.setLocationRelativeTo(null);
        this.vista.setTitle("Biblioteca Fastdevelopment | Gestión añadir libros a venta");
        this.vista.txt_tituloTabla.setText("Añadir libros a venta: " + this.modelVenta.getCodigo_venta());
        ...
        this.vista.campo_numSerie.setText(String.valueOf(modelVenta.getCodigo_venta()));
    }  
    ...
    private void asociar() {
        try {
            if (!libTemp.equals(null)) {
                if (sql.insert(modelVenta.getId(), libTemp.getId())) {
    ...
    private void quitar() {
        try {
            if (!libTemp.equals(null)) {
                if (sql.delete(modelVenta.getId(), libTemp.getId())) {
