 ...
 */
public class AsociarLibrosVentasSql extends ConecctSql {

    private PreparedStatement ps;
    private ResultSet rs;

    public boolean insert(int idvent, int idlib) {
        Connection con = null;
        try {
            String sql = "INSERT INTO `libros_en_ventas`(`idLibros_en_Ventas`, `Ventass_idVenta`, `Libros_idLibro`) "
                    + "VALUES (0,?,?)";
            con = getConection();
            ...
            if(ps.executeUpdate() != 0){
                ...
                String sqlUpdate = "UPDATE `libros` SET Estados_idEstado = 19 WHERE idLibro = "+idlib+"";
    ...
    public int getCantidad(int id) {
        ...
        try {
            ...
            String sql = "SELECT COUNT(Ventass_idVenta) FROM libros_en_ventas WHERE Ventass_idVenta = ?";
            con = getConection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, id);
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                numero = rs.getInt("COUNT(Ventass_idVenta)");
            }
            return numero;
        } catch (Exception e) {
    ...
    public int[] idLibrosSinAñadir(int idventa, int cantidadRegistros) {
        Connection con = getConection();
        int listadoIds[] = new int[cantidadRegistros];

        try {

            String sql = "SELECT Libros_idLibro FROM libros_en_ventas WHERE Ventass_idVenta = ?";
            con = getConection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, idventa);
    ...
    public ArrayList<Libro> milistadoLibros(int[] listado, boolean igualdad, int id) {
        ...
        try {
            ...
            if (igualdad) {
                ...
                sql = "SELECT lib.idLibro, lib.isbn, lib.num_serie, lib.nombre FROM libros_en_ventas libVen INNER JOIN libros lib ON libVen.Libros_idLibro = lib.idLibro INNER JOIN ventass vent ON libVen.Ventass_idVenta = vent.idVenta WHERE Ventass_idVenta = "+id+"";
    ...
    public boolean delete(int idv, int idl) {
        Connection con = null;
        try {
            String sql = "DELETE FROM `libros_en_ventas` WHERE Ventass_idVenta = "+idv+" AND Libros_idLibro = "+idl+"";
            con = getConection();
            ps = con.prepareStatement(sql); 
